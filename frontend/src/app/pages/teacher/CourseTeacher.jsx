      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {courses.length === 0 && !error && (
            <div className="col-span-full text-center text-slate-500 py-10">No courses found. Add a new course to get started.</div>
          )}
          {courses.map((course) => (
            <div key={course.chap_id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col hover:shadow-md transition-shadow">
              <div className="p-5">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    <h3 className="font-semibold text-lg text-slate-800">{course.chap_name}</h3>
                    <span className="bg-blue-100 text-zense-navy text-xs font-semibold px-2 py-0.5 rounded-md">
                      {course.chap_id}
                    </span>
                  </div>
                </div>
                ...
                <p className="text-sm text-slate-600 mb-6 min-h-[40px]">
                  {course.chap_desc || 'No description provided.'}
                </p>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="bg-slate-50 rounded-xl p-4 flex flex-col items-center justify-center text-center">
                    <HelpCircle size={20} className="text-zense-navy mb-2" />
                    <span className="text-xl font-bold text-slate-800">{course.questions_count}</span>
                    <span className="text-xs text-slate-500">Questions Bank</span>
                  </div>
                  <div className="bg-slate-50 rounded-xl p-4 flex flex-col items-center justify-center text-center">
                    <FileText size={20} className="text-zense-navy mb-2" />
                    <span className="text-xl font-bold text-slate-800">{course.total_exams}</span>
                    <span className="text-xs text-slate-500">Total Exams</span>
                  </div>
                </div>

                <div className="flex gap-2">
                  <button onClick={() => navigate('/teacher/questions')} className="flex-1 bg-zense-navy text-white text-sm font-medium py-2 rounded-lg hover:bg-blue-900 transition-colors">
                    View Course
                  </button>
                  <button onClick={() => openEditModal(course)} className="p-2 text-slate-400 hover:text-blue-500 hover:bg-blue-50 border border-slate-200 rounded-lg transition-colors">
                    <Edit3 size={18} />
                  </button>
                  <button onClick={() => handleDelete(course.chap_id)} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 border border-slate-200 rounded-lg transition-colors">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-lg mx-4 shadow-xl overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <div>
                <h2 className="text-xl font-bold text-slate-800">{isEditing ? 'Edit Course' : 'Add New Course'}</h2>
                <p className="text-sm text-slate-500 mt-1">{isEditing ? 'Update course details' : 'Create a new course for your students'}</p>
              </div>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label htmlFor="chap_name" className="block text-sm font-medium text-slate-700 mb-1.5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        onChange={handleInputChange}
                        className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm appearance-none bg-white disabled:bg-slate-100 disabled:text-slate-500"
                        required
                        disabled={isEditing}
                      >
                  ...
                  <div>
                    ...
                    <div className="w-full px-4 py-2.5 rounded-xl border border-slate-100 bg-slate-50 text-slate-500 text-sm">
                ...
                <div>
                  <label htmlFor="chap_id" className="block text-sm font-medium text-slate-700 mb-1.5">
                    Course Code (รหัสคอร์ส)
                  </label>
                  <input
                    type="text"
                    id="chap_id"
                ...
                <div>
                  <label htmlFor="chap_desc" className="block text-sm font-medium text-slate-700 mb-1.5">

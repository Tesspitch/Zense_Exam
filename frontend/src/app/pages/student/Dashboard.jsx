    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <button 
          ...
          className="px-6 py-2 bg-zense-navy text-white rounded-xl hover:bg-slate-800 transition-colors"
        >
          ลองใหม่อีกครั้ง
        </button>
      </div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div 
        ...
        className="bg-white rounded-2xl border border-blue-100 p-6 shadow-sm flex items-center gap-5 hover:border-blue-300 transition-all cursor-pointer group hover:shadow-md"
      >
        <div className="bg-zense-navy w-14 h-14 rounded-xl flex items-center justify-center text-white group-hover:scale-105 transition-transform shadow-md">
          <ArrowRight size={24} />
        </div>
        <div>
          <h3 className="text-lg font-bold text-slate-800">Join Exam Room</h3>
          <p className="text-sm text-slate-500">Enter exam code to start your assessment</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        <h3 className="text-lg font-bold text-slate-800 mb-6">Recent Scores</h3>
        
        {data?.recent_scores && data.recent_scores.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {data.recent_scores.map((score, index) => (
              <div key={index} className="border border-slate-100 rounded-xl p-5 hover:shadow-md transition-all bg-slate-50/30">
...
const StatCard = ({ title, value, icon }) => (
  <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col justify-between h-32">
    ...
    <div className="text-4xl font-bold text-slate-800 mt-4">{value}</div>
  </div>
);
